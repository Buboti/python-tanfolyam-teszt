...
def get_word_list(difficulty: str):
    """
    The word list with different difficulty level
...
def choose_difficulty() -> str:
    ...
    """
    while True:
        diff = input('Valasszon nehezsegi szintet (easy, medium, hard): ').lower()
        if diff in ['easy', 'medium', 'hard']:
            return diff
        print('Ervenytelen valasztas! Csak easy, medium, hard adhato meg!')
...
def draw_hangman(tries_left: int):
    ...
    print(stages[max(0, min(6, 6 - tries_left))])  # biztonságos indexelés
...
def display_statistics(stats: dict):
    ...
    print(f"Hatralevo probalkozasok a gyoztes jatekokban: {stats['tries_remaining']}")


def update_stats(stats: dict, won: bool, word: str, tries_left: int):
    stats['total_games'] += 1
    if won:
        stats['games_won'] += 1
        ...
        while not game_over:
            ...
            if guess in word:
                ...
                if is_game_won(hidden_word):
                    print(f'Gratulalunk! Megnyerted a jatekot, helyesen kitalaltad a {word} szot.')
                    game_over = True
                    update_stats(stats, True, word, tries_left)
            else:
                incorrect_guesses.append(guess)
                tries_left -= 1
                if is_game_lost(tries_left):
                    draw_hangman(tries_left)
                    print(f'Sajnalom, vesztettel! A szo a {word} volt!')
                    game_over = True
                    update_stats(stats, False, word, tries_left)

        # ✅ statisztika csak a játék végén
