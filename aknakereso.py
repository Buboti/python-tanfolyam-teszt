...
def calculate_numbers(grid):
    """
    Kell egy lista az osszes irannyal. A lista elemei pozicios parameter parok, puple parok (dr, dc)
    * 'dr' = A sorban torteno elmozdulas (delta row)
    * 'dc' = A oszlopban torteno elmozdulas (delta column)
    ...
    [(-1,-1), (-1,0), (-1,1)
      (0,-1), (0,0),   (0,1)
      (1,-1), (1,0),   (1,1)]
...
def display_grid(visible_grid):
    ...
    for idx, row in enumerate(visible_grid):
        print(f'{idx} │ ' + ''.join(
            colorize(cell, NUMBER_COLORS.get(cell, Fore.LIGHTCYAN_EX if cell == 'F' else Fore.RED if cell == 'M' else EMPTY_CELL))
            for cell in row
        ))

def reveal_cell_with_recursion(grid, visible_gird, row, col):
    if visible_gird[row][col] != EMPTY_CELL:
        return False
    ...
    def flood_fill(r, c):
        if not (0 <= r < GRID_SIZE and 0 <= c < GRID_SIZE):
            return
        if visible_gird[r][c] != EMPTY_CELL or grid[r][c] == 'M ':
            return
...
def reveal_cell ( grid, visible_gird, row, col ):
    ...
    if grid[row][col] == EMPTY_CELL:
        visible_gird[row][col] = '*'
    else:
        visible_gird[row][col] = grid[row][col]
    ...
    while stack:
        ...
        if visible_gird[r][c] not in [EMPTY_CELL, '*'] or grid[r][c] == 'M':
            continue
...
def play():
    ...
    while revealed_cells < total_cells:
        ...
        print('\nUtmutato')
        print('- R <sor> <oszlop>: Mezo feltarasa (pl.: ’R 3 4’ => feltarjuk a 3. sor 4.oszlopat ')
        print('- F <sor> <oszlop>: Zaszlo leszurasa (pl.: ’F 3 4’ => zaszlot rak ide: a 3. sor 4.oszlopat ')
        print('A koordinatak tartomanya 0-tol', GRID_SIZE - 1, '-ig terjednek')
        print('A zaszlokat az aknak gyanus helyeire teheted')
        print('Ha aknara lepsz a jatek veget er, ha megvan mind akna, akkor is vege a jateknak')

        command = input ('Adj meg egy parancsot (pl.: ’R 3 4’): ').strip().upper()
        parts = command.split()

        if len (parts) != 3 or parts[0] not in ['R', 'F']:
            print ('Erevenytelen parancs! Hasznal "R" vagy "F" parancsot a megfelelo formatumban')
            continue
        ...
        if not (0 <= row < GRID_SIZE and 0 <= col < GRID_SIZE):
            print('Ervenytelen koordinatakat adtal meg te barom. A sorok és az oszlopok tartománya 0-to, kezodik és', GRID_SIZE -1, '-ig tart')
            continue
        ...
        if action == 'F':
            ...
            if hit_mine:
                print('\nBUMM!! baszhatod, aknara leptel!')
                for r, c in mines:
                    visible_grid[r][c] = 'M'
                display_grid(visible_grid)
                print('Vege a jateknak!')
                return
            revealed_cells = sum(1 for row in range(GRID_SIZE) for col in range(GRID_SIZE) for col in range(GRID_SIZE)
