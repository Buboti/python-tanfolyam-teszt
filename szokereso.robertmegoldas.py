...
def get_word_selection(grid):
    ...
    print(' - Vegpontonkent adja meg a koodinatakat (pl.: veg sor: 2 oszlop: 7')
    print ('Pelda a -SZINTAXIS- szohoz a kezdo koordinata (1,1) veg koordinata: (1,8) ')
    size = len(grid)
...
def check_and_mark_word_in_grid(grid, word, start_row, start_col, end_row, end_col):
    ...
    # hossz ellenőrzés
    if abs(end_row - start_row) + 1 != word_length and abs(end_col - start_col) + 1 != word_length:
        return False
    ...
    # ha jó, akkor kisbetűsre váltja (zölden jelzi)
    for i in range(word_length):
        r = start_row + i * row_step
        c = start_col + i * col_step
...
def main():
    ...
    while len(found_words) < len(word_list):
        ...
        found = False
        for word in word_list:
            if word not in found_words and check_and_mark_word_in_grid(grid, word, start_row, start_col, end_row,
                                                                       end_col):
