...
def run_sync_io(num_task: int =10):
    ...
    print(f'[SZINKRON] ossz futttasi ido: {total:.2f} mp')
...
async def run_async_io(num_task: int = 10):
    ...
    total = time.time() - start
    print(f'[ASSZINKRON] ossz futttasi ido: {total:.2f} mp')
...
def run_threaded_io(num_task: int = 10):
    ...
    threads = []
    for i in range(1, num_task + 1):
        t = threading.Thread(target=fake_io_task, args=(i,1.0, 'THREADES'))
        threads.append(t)
        t.start()
    ...
    total = time.time() - start
    print(f'[THREADES] ossz futttasi ido: {total:.2f} mp')
