...
def input_player():
    ...
    while not (marker == 'X' or marker =='O'):
        marker = input(PLAYER1_NAME + 'mivel szeretnel jataszni, X vagy O: ').upper
    if marker == 'X':
        return ('X', 'O')
    return ('O', 'X')
...
def check_winning(table, marker):
    ...
    (table[7] == marker and table[4] == marker and table[1] == marker) or
    (table[8] == marker and table[5] == marker and table[2] == marker) or
    (table[9] == marker and table[6] == marker and table[3] == marker) or
    (table[7] == marker and table[4] == marker and table[1] == marker) or
    (table[7] == marker and table[5] == marker and table[3] == marker) or
    (table[9] == marker and table[5] == marker and table[1] == marker)
    )
...
def run_player_logic(marker, table, next_player):
    ...
    if check_winning(table, marker):
        ...
        if is_table_full(table):
            show_table(table)
            print('A jatek dontetlen,senki sem nyert!')
            game_on = False
        else:
            who = next_player
...
while True:
    ...
    repeat_game = input('Szeretne ujra jatszni? Irja be, hogy igen vagy nem: ').lower().startswith('i')
    if not repeat_game:
        break
